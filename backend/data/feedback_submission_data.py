...
class FeedbackSubmissionData(JsonRecordFile):
    ...
    def getSubmissionByStudentId(self, assignment_record: dict,
                                 student_Id: str):
        submission_index = -1
    ...
    def updateSubmission(self, assignment_record: dict, submission_index: int,
                         student_submission: StudentSubmission):
        assignment_record["submissions"][submission_index] = \
            student_submission.model_dump(
        )

    def addOrUpdate(self, assignment_record: dict,
                    student_submission: StudentSubmission):
        submission_index, _ = self.getSubmissionByStudentId(
            assignment_record, student_submission.studentId)
        if submission_index >= 0:
            self.updateSubmission(
                assignment_record, submission_index, student_submission)
        else:
            assignment_record["submissions"].append(
                student_submission.model_dump())

    def addSubmission(self, new_submission: Submission):
        try:
            # self.all_records.append(new_submission.model_dump())
            assignment_record = self.getRecordByAssignmentId(
