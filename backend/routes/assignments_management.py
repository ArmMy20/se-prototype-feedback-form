...
async def post_assignment(new_assignment: Assignment, current_user: Annotated[UserRole, Depends(get_current_user)]):  
    try:  
...
async def get_assignments(current_user: Annotated[UserRole, Depends(get_current_user)]):
